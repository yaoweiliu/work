...
static irqreturn_t priv_interrupt(int irq, struct uio_info *dev)
{
...
typedef struct button_irq_desc {
	int irq;
	int num;
	char *name;
}button_irq_desc_t;

static button_irq_desc_t button_irqs[] = {
	{IRQ_EINT8, 1, "KEY0"},
	{IRQ_EINT11, 2, "KEY1"},
	{IRQ_EINT13, 3, "KEY2"},
	{IRQ_EINT14, 4, "KEY3"},
...
{
	button_irq_desc_t *button_irqs = (button_irq_desc_t *)dev;

	unsigned long *ret_val_add = (unsigned long *)(kpart_info.mem[1].addr);
	*ret_val_add = button_irqs->num;

	printk("%s: %d is being pressed...\n", __func__, button_irqs->num);
...
static int drv_kpart_probe(struct platform_device *pdev)
{
	...
	if(!kpart_info.mem[1].addr)
		return -ENOMEM;
	kpart_info.mem[1].memtype = UIO_MEM_LOGICAL;
	kpart_info.mem[1].size = 4;

	ret_val_add = (unsigned long *)(kpart_info.mem[1].addr);
	*ret_val_add = 999;
