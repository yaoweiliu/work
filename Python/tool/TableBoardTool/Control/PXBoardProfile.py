...
class PXBoardProfile(ProfileBase):
    def __init__(self, name, view):
        super(PXBoardProfile, self).__init__(name, view)

        self.connect(view, QtCore.SIGNAL("PXBoardStartTest(PyQt_PyObject)"), self.test)
    ...
    def test(self):
        ...
        serialOpera.sendData([0x55, 0x05, 0xA0, 0x00, 0xA0])
    ...
    def checkLink(self):
        ...
        if link == "px_link_is_ok":
            logMessage("貔貅电路板通讯链路正常!")
            self.view.ui.textEdit.append(u"貔貅电路板通讯链路正常!")
            ...
            logMessage("貔貅电路板通讯链路异常，请检查通讯链路!")
            self.view.ui.textEdit.append(u"貔貅电路板通讯链路异常，请检查通讯链路!")
            self.view.ui.pushButton.setDisabled(False)

    def checkData(self):
        #data1,data2,data3 = serialOpera.revData()
        d1 = OrderedDict([("releaseStatIn",1),("touchStatIn",1),("encoderAIn",1),("encoderBIn",1),("serialTxIn",1),("elevHallAIn",1),("elevHallBIn",1),("footHomeIn",0)])
        d2 = OrderedDict([("footFuncIn",0),("relayCtrlIn",1),("petExoffIn",1),("stepPulseIn",1),("stepDirIn",1),("servoPulseIn",1),("servoDirIn",1),("stepClutchIn",1)])
        ...
        d9 = OrderedDict([("elev1OvcStat",1),("servoAlarmStatOut",1),("servoPrepareStatOut",1),("servoWorkStatOut",1),("serialRxOut",1),("hpotMisoOut",1),("vpotMisoOut",1),("framMemory",1)])
            ...
            for b in range(8):
                if (int(data[i], 16) & 0xff) & (0x1 << b) == (valLt[b] << b):
                    ...
                    if keyLt[b] == "none" or keyLt[b] == "none2" or keyLt[b] == "none3" or keyLt[b] == "none4":
                        ...
                        logMessage(("貔貅电路板: %s link error" % keyLt[b]))
                        success = False
                        #self.view.ui.label_2.setText(u"测试失败，请查看log文件信息")
                        self.view.ui.textEdit.append((u"貔貅电路板: %s link error" % keyLt[b]))
                        self.view.ui.pushButton.setDisabled(False)
        if success:
            #self.view.ui.label_2.setText(u"测试通过")
            self.view.ui.textEdit.append(u"貔貅电路板测试通过!")
            self.view.ui.pushButton.setDisabled(False)
            logMessage("貔貅电路板测试通过!")
